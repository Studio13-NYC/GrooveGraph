...
export async function runTripletExplorationPipeline(
  ...
  console.log(`${LOG_PREFIX} Sending POST to ${url} (timeout ${timeoutMs}ms)...`);
  const response = await withRetry(
    async () => {
      ...
      console.log(`${LOG_PREFIX} Response received status=${res.status}`);
  ...
  console.log(`${LOG_PREFIX} Parsing JSON body...`);
  const payload = (await response.json()) as ResponsesApiPayload;
  console.log(`${LOG_PREFIX} Payload keys:`, Object.keys(payload ?? {}), "output length:", Array.isArray((payload as ResponsesApiPayload)?.output) ? (payload as ResponsesApiPayload).output!.length : "n/a");
  ...
  const rawEdgeCount = Array.isArray((withProvenance as Record<string, unknown>)?.edgeCandidates)
    ? ((withProvenance as Record<string, unknown>).edgeCandidates as unknown[]).length
    : 0;
  console.log(`${LOG_PREFIX} Raw bundle before validation: edgeCandidates=${rawEdgeCount}`);
