...
export async function runLlmOnlyPipeline(
  ...
  console.log(`${LOG_PREFIX} Sending POST to ${url}...`);
  ...
  console.log(`${LOG_PREFIX} Response received status=${response.status}`);
  if (!response.ok) {
  ...
  console.log(`${LOG_PREFIX} Payload keys:`, Object.keys(payload ?? {}), "output length:", Array.isArray(payload?.output) ? payload.output.length : "n/a");
