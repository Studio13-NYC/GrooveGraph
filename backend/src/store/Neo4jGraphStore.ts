...
export class Neo4jGraphStore implements GraphStore {
  ...
  async findProposedNodes(): Promise<GraphNode[]> {
    return this.read(async (session) => {
      const result = await session.run(
        `MATCH (n:GraphEntity) WHERE n.__meta IS NOT NULL AND n.__meta CONTAINS $proposedFlag
         RETURN n, labels(n) AS labels ORDER BY n.id`,
        { proposedFlag: '"proposed":true' }
      );
      return result.records.map((record) => {
        const node = toNodeRecord(record);
        return new StoredNode(node.id, node.labels, node.properties, node.meta);
  ...
  async findProposedEdges(): Promise<GraphEdge[]> {
    return this.read(async (session) => {
      const result = await session.run(
        `MATCH (from:GraphEntity)-[r]->(to:GraphEntity)
         WHERE r.__meta IS NOT NULL AND r.__meta CONTAINS $proposedFlag
         RETURN r, type(r) AS type, from.id AS fromNodeId, to.id AS toNodeId ORDER BY r.id`,
        { proposedFlag: '"proposed":true' }
      );
      return result.records.map((record) => {
        const edge = toEdgeRecord(record);
