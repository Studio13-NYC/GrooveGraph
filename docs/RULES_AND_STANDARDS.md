...
| **chat-graph-interaction-stability.mdc** | Chat graph UX stability: typing must not move graph; keep drag/zoom/pan enabled; avoid relayout churn from unstable React props. | `frontend/app/components/**/*.tsx` |
...
- Keep Cytoscape interaction predictable in chat workflows: typing must not trigger graph movement, and mouse drag/zoom/pan must remain available.
