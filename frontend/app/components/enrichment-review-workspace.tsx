...
export function EnrichmentReviewWorkspace() {
  ...
  const loadSession = useCallback(async (sessionId: string) => {
    setWorking(true);
    try {
      const response = await fetch(`${getApiBase()}/api/enrich/review-session/${sessionId}`, { credentials: "include", headers: getAuthHeaders() });
  ...
  async function runQuickRunthrough() {
    ...
    try {
      ...
      if (!response.ok) {
        const errMsg = data.error ?? "Extract failed";
        setRunthroughResult({
          ok: false,
          message: errMsg,
          detail: `${response.status}${data.error ? ` — ${data.error}` : ""}`,
        });
        return;
      }
      const meta = data.runMetadata;
