...
export async function runTripletExplorationPipeline(
  ...
  try {
    ...
    if (isTimeout) {
      throw new Error(
        `Triplet exploration timed out after ${timeoutMs / 1000}s waiting for GPT response. GPT-5.4 reasoning can be slow. Set TRIPLET_LLM_TIMEOUT_MS or ENRICHMENT_LLM_TIMEOUT_MS (ms) to increase.`
      );
    }
    throw fetchErr;
  }
